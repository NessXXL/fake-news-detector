...
def main():
    ...
    train = pd.concat(
        [k_train] + [liar_train] * 3 + [f_train] * 3, ignore_index=True
    ).sample(frac=1, random_state=SEED).reset_index(drop=True)
    val = pd.concat([k_val, liar_val, f_val], ignore_index=True)
    ...
    print(f"train: {len(train)}  (kaggle {len(k_train)}, liar x3 {len(liar_train)*3}, fnn x3 {len(f_train)*3})")
    print(f"val: {len(val)}   tests: kaggle {len(k_test)}, liar {len(liar_test)}, fnn {len(f_test)}")
